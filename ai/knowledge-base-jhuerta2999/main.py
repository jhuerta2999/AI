...
class KBTest(unittest.TestCase):
    ...
    #When there is a direct match between facts and an empty LOB, then it will be asserted to false even though there is a match
    def test6(self):
        ask1 = read.parse_input("fact: (color pyramid4 red)")
        print(' Asking if', ask1)
        answer = self.KB.kb_ask(ask1)
        self.assertFalse(answer[0].bindings, [])

    #Binds facts containing pyramid1 and blue, and because there is a LOB that match this could not be asserted to false because this binding is true
    def test7(self):
        ask1 = read.parse_input("fact: (color pyramid1 ?Y)")
        print(' Asking if', ask1)
        answer = self.KB.kb_ask(ask1)
        print(answer)
        self.assertTrue(str(answer[0]), "?Y : blue")
