...
In this assignment, you are going to create a basic knowledge base (KB) to store and retrieve facts. The facts will be statements that includes predicates (e.g., Color, Size, Inst) that relate objects together.  For example:
...
To test this assignment, we'll create several testing files that contain a bunch of facts (similar to the ones provided). Each fact will be asserted one-by-one into the KB. After asserting the facts, a query will be constructed and the KB will be asked for a fact.  *We also recommend you make your own testing files, and feel free to share them on Piazza.**. When sharing tests, please provide your rationale to the test, explain what you hope to test and/or how you developed the test.


### Your tests

In addition to the test that are provided to you, you need to write two additional tests.  Add your additional tests to the end of `main.py`.
...
1.  Statements in `statements_kb.txt` describe blocks in various shapes and colors.  What facts would you add to describe a yellow sphere?


2.  Assume the KB has the following facts?
...
Given what you have implemented in this assignment, if we ask the KB `(isa Dog ?x)`, what will get returned?
...
*You can ignore this for now.  We will use it the next assignment.*
...
Represents Binding(s) used while matching two statements.  As the result of matching two statements, a consistent set of bindings describe the assignment (aka binding) of each variable.
...
A collection of Bindings.  Since each Bindings is a collection of assignments for each variable, a ListOfBindings is a group of there where each Bindings represent alternative sets of bindings between two statements.
...
This file has no classes but defines useful helper functions.  Here are two of the especially useful functions.
...
- `factq(element)` (`(Statement) => bool`) - check if the provided element is an instance of a Fact
...
This file defines the class you need to implement, KnowledgeBase.
