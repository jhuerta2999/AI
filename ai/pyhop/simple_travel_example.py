...
def request_lyft(state, a, x):
    state.loc['lyft'] = x
    return state

def ride_lyft(state, a, x, y):
    if state.loc['lyft']==x and state.loc[a]==x:
        state.loc['lyft'] = y
        state.loc[a] = y
        state.cc[a] += lyft_rate(state.dist[x][y])
        return state

    else: return False

pyhop.declare_operators(walk, call_taxi, ride_taxi, pay_driver, ride_lyft, request_lyft)
print('')
...
def travel_by_lyft(state,a,x,y):    
	if state.lyft_app[a]:        
		return [('request_lyft',a,x), ('ride_lyft',a,x,y)]    
	return False



pyhop.declare_methods('travel',travel_by_foot,travel_by_taxi, travel_by_lyft)
print('')
pyhop.print_methods()
