...
In this assignment, you are going to extend a knowledge base (KB) and an inference engine. The knowledge base supports three main interfaces: `Assert`, `Retract`, and `Ask`.
...
There are a few data files: `statements_kb.txt`, `statements_kb2.txt`, `statements_kb3.txt`, `statements_kb4.txt`.  These files contain the facts and rules to be inserted into the KB. The provided tests use `statements_kb4.txt`, but you may use any of the files to generate your own tests.
...
To test this assignment, we have created several testing files that contain a bunch of facts and rules (similar to the ones provided). Each fact/rule will be asserted one-by-one into the KB. Other files containing more facts/rules will be used to test the `Retract` operation, and make sure it worked correctly. *We recommend that you carefully test each function and make sure you understand the correct inputs and outputs*. 



### Your tests

In addition to the test that are provided to you, you need to write two additional tests.  Add your additional tests to the end of `main.py`.  For each test, please provide your rationale to the test; explain what you hope to test and/or how you developed the test.  Since `kb_retract` is a little more complicated and requires getting the support structure setup correctly in the inference engine, **at least one of your tests must test `kb_retract`**.  It is also suggested that you write your tests *before* you finish your implementation.
